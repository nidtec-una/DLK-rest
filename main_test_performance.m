...
% --- Parámetros ---
[A, b] = construir_matriz.cargar_matices('mat_collection_test_full/sherman5');
title_label = 'sherman5';
m0 = 30;
m = 110;
%[A,b] = construir_matriz.genMatrixZNn(50,100);
...
% -- Grafico de comparacion de metodos
graficos.show_results_custom(R_list, ['Comparison of methods - ', title_label],tol);
m_limits = [2, floor(size(A,2)*0.1)];

% -- Grafico performance DMDc-LQR-GMRES
R_gmres_dmdc = {
    {R3, error_per_snp3,     'DMDcR-LQR-GMRES', '--^', [0.4, 0.4, 0.4]},
};
graficos.show_results_with_m(R_gmres_dmdc, ['Performance of DMDc-LQR-GMRES($m$) - ', title_label], m_limits, m_values3, p_values3);

% -- Grafico performance DMDc-LQR-LGMRES
R_gmres_dmdc = {
    {R4, error_per_snp4,     'DMDcR-LQR-LGMRES', '-d', [0.6, 0.6, 0.6]},
};
graficos.show_results_with_m(R_gmres_dmdc, ['Performance of DMDc-LQR-LGMRES($m,\ell$) - ', title_label], m_limits, m_values4, p_values4);
