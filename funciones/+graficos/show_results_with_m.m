function show_results_with_m(R_list, title_label, m_limits, m_values, p_values) 
...
for i = 1:num_series
    ...
    if numel(R) > maxX+1, R = R(1:maxX+1); end
...
all_errors = [];
for i = 1:num_series, all_errors = [all_errors, R_list{i}{2}(:)']; end
min_err = min(all_errors); max_err = max(all_errors);
...
exportgraphics(fig, fullfile('test_performance', [clean_title, '_with_m.eps']), 'ContentType', 'vector');
