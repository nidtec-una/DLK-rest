function graficar_3D(data_cell, p)

    r_sphere = 20;
    % ============================
    % FIGURA – Proporciones SIAM
    % ============================
    ...
    % ============================
    % Determinar límites globales para consistencia
    % ============================
    ...
    % Gradiente único
    g = gray(256); g = flipud(g(50:end-30,:)); 
    colormap(fig, g);
    ...
    for i = 1:length(data_cell)
        ...
        p_opt = T.p(idx); q_opt = T.q(idx); t_opt = T.t(idx);
        plot3(ax, p_opt, q_opt, t_opt, 'ko', 'MarkerSize', 8, 'LineWidth', 1.2);

        % ============================
        % Flecha tipo Bezier
        % ============================
        p0 = [max_axes*0.25, max_axes*0.25, max_axes*1.1]; % origen de la flecha
        ...
        % Límites uniformes
        % ============================
        xlim(ax,[0 max_axes]); ylim(ax,[0 max_axes]); zlim(ax,[0 max_axes]);
    end
    ...
    % ============================
    % Título global (manual, sobre la figura)
    % ============================
    
    %axes(fig, 'Position',[0 0 1 1], 'Visible','off');  % Ejes invisibles para texto
