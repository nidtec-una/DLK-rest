function [xf, R, tot_cycles, m_values, p_values, erro_per_snp] = GMRES_DMDc_LQR_fixp(A, b, x0, m0, tol, cycleMax,alpha_LQR,p,q,t)

%% Valores por defecto
if nargin < 10, t = 6; end
if nargin < 9, q = 5; end
if nargin < 8, p = 5; end
if nargin < 7, alpha_LQR = 1e-4; end
...
while tot_cycles <= cycleMax && (isempty(R) || R(end) >= tol)
    ...
    m = ley_de_control.updatebyLQR(Hbar, Bbar, Xhat_p(:,end), m, alpha_LQR);
    % Acotado de m
