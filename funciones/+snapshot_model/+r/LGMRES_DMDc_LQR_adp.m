function [xf, R, tot_cycles, m_values, p_values, error_per_snp, Disp] = GMRES_DMDc_LQR_adp(A, b, x0, m0, l, tol, cycleMax, p_min, p_max, threshSVD,alpha_LQR)
...
%% Valores por defecto
if nargin < 11, alpha_LQR = eps; end
if nargin < 10, threshSVD = eps; end
if nargin < 9, p_max = 6; end
if nargin < 8, p_min = 3; end
if nargin < 7, cycleMax = 300; end
...
while tot_cycles <= cycleMax && (isempty(R) || R(end)/normR0 >= tol)
   ...
   %% Construir snapshot con LGMRES
    ...
    dim_sp = ley_de_control.updatebyLQR(Hbar, Bbar, Xhat_p(:,end), m, alpha_LQR);
    % Acotado de m
