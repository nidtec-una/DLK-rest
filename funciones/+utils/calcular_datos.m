function T = calcular_datos(A, b, p)
    % 1. Generar mallado y filtrar puntos válidos
    [P_grid, Q_grid, T_grid] = meshgrid(1:10, 1:10, 1:10);
    mask = (Q_grid <= P_grid) & (T_grid <= P_grid + 1);
    P = P_grid(mask); Q = Q_grid(mask); T_vec = T_grid(mask);
    
    num_pts = length(P);
    C_mean = zeros(num_pts, 1); C_rmssd = zeros(num_pts, 1); valid = false(num_pts, 1);
    
    % 2. Inicializar barra de progreso con el método actual
    msg = sprintf('Calculando %s (alpha = %.0e)', p.method, p.current_alpha);
    h = waitbar(0, msg);
    ...
    for k = 1:num_pts
        try
            ...
            % Error de convergencia o interrupción
        end
        ...
        % Opcional: Abortar si se cierra la waitbar
        if ~ishandle(h), break; end
    end
    
    % 4. Cerrar barra y empaquetar resultados
    ...
    % 5. Cálculo de métricas (Protección contra tabla vacía)
    if ~isempty(T)
